...
namespace Display
{
  void MenuTileNavigation::displayMenuNavigation()
  {
    ...
    float inactiveSelectionDimmingScale = contextManager->stateManager.current() == Engine::SystemState::MenuHome ? 1.0f : 0.3f;
    ...
    displayIndex = 0;
    inactiveSelectionDimmingScale = (contextManager->stateManager.current() == Engine::SystemState::MenuGames || contextManager->stateManager.current() == Engine::SystemState::MenuScenes) ? 1.0f : 0.3f;
    uint8_t gamesOrScenesAvailable = contextManager->stateManager.getUserMenuChoice() == Engine::MainMenuSelection::Games ? numGames : numScenes;
    ...
    for (uint8_t modeIdx = 0; modeIdx < gamesOrScenesAvailable; ++modeIdx)
    {
      for (size_t i = 0; i < menuTileWidth; ++i)
      {
        ...
        if (contextManager->stateManager.current() == Engine::SystemState::MenuGames && modeIdx == gameSelected)
          contextManager->leds.buffer[displayIndex] = Lights::Color{Lights::ColorCode::ThemeBlue} * blend;
        else if (contextManager->stateManager.current() == Engine::SystemState::MenuScenes && modeIdx == sceneSelected)
