#include "lights/led-strip2.h"

namespace Lights
{
  LedStrip2::LedStrip2()
  {
#ifdef RELEASE
    FastLED.addLeds<WS2815, 18, GRB>(static_cast<Color2 *>(buffer), size());
    // TODO: Test if this now works on bare metal with buffer istead of static_casting it to Color2 *.
    // FastLED.setCorrection(0xFFB0F0);
    // FastLED.setTemperature(0xFFE0A0);
    reset();
#endif
  }

  Color2 *LedStrip2::getRawColors()
  {
    return static_cast<Color2 *>(buffer);
  }

  void LedStrip2::reset()
  {
    for (uint16_t i = 0; i < size(); i++)
    {
      buffer[i] = 0;
    }
  }

  void LedStrip2::adjustLuminance()
  {
#ifdef RELEASE
    luminance.adjustLuminance();
    float brightnessScale = static_cast<float>(luminance.getLuminance()) / LedLuminance::MAX_LED_BRIGHTNESS;
    constexpr float redGain = 0.90f;
    constexpr float greenGain = 0.75f;
    constexpr float blueGain = 0.60f;

    for (uint16_t i = 0; i < size(); ++i)
    {
      Color2 &c = buffer[i];

      float r = c.r * brightnessScale;
      float g = c.g * brightnessScale;
      float b = c.b * brightnessScale;

      r *= redGain;
      g *= greenGain;
      b *= blueGain;

      r = std::clamp(r, 0.0f, 255.0f);
      g = std::clamp(g, 0.0f, 255.0f);
      b = std::clamp(b, 0.0f, 255.0f);

      c.r = LedLuminance::applyGamma(static_cast<uint8_t>(r));
      c.g = LedLuminance::applyGamma(static_cast<uint8_t>(g));
      c.b = LedLuminance::applyGamma(static_cast<uint8_t>(b));
    }
#endif
  }
}